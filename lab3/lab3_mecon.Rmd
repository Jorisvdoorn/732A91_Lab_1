...
set.seed(12345)
RWMsampler = function(LogPost, tmax,c, X0, proposeSigma,...){
  t=1
  X=matrix(X0,nrow=tmax+1, ncol=length(X0))
  alphaVec = c()
  while(t<=tmax){
    Y=as.vector(rmvnorm(1, mean=X[t,], sigma=c*proposeSigma))
    U=runif(1,min=0,max=1)
    
    alpha = min(1, exp(LogPost(Y,...)-LogPost(X[t,],...)) )
    ##use exp to cancel out the log
  ...
  return(list("RWMsample"=X,"alphas="=alphaVec))
}
tmax=1000
c=0.6 # the smaller the quicker it walks
X0=initValue
proposeSigma=hessianBeta

MCMC=RWMsampler(LogPost, tmax, c, X0, proposeSigma,y, x, mu, sigma)

# plot the 9 samples covaraite from MCMC
par(mfrow=c(3,3))
for(i in 1:9){
  plot(MCMC$RWMsample[,i],type="l",xlab=names(data)[i+1])
}
```
...
mcmc_beta = MCMC$RWMsample

predDist = c()
set.seed(12345)
for(i in 1:dim(mcmc_beta)[1]){
  predDist[i] = rpois(1,exp(t(x_given) %*% mcmc_beta[i,]))
}

values=unique(predDist)
